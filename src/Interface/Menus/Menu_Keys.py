...
class KeysMenu(BaseMenu):
    ...
    def setup_buttons(self):
        ...
        self.buttons_keys['right1_button'] = factory.create_TextButton({"text": self.config.config_keys["Name_key7"],"position": (175,self.HEIGHT/2-50),"command1":lambda:self._change_keys("key_right","Name_key7",self.buttons_keys["right1_button"])})
        ...
        self.buttons_keys['left2_button'] = factory.create_TextButton({"text": self.config.config_keys["Name_key6"],"position": (self.WIDTH-50,self.HEIGHT/2-50),"command1":lambda:self._change_keys("key_left2","Name_key6",self.buttons_keys["left2_button"])})
        ...
        self.buttons['save'] = factory.create_TextButton({"text": "Save config","position": (self.WIDTH/2,self.HEIGHT-85),"command1":self.config.save_config})
        self.buttons['default'] = factory.create_TextButton({"text": "Default config","position": (self.WIDTH/2-40,self.HEIGHT-50),"command1":lambda:(self.config.config(keys=True),self.change_mains({"main":6,"command":self.setup_buttons}))})
